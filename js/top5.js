...
function updateTop5CountryText(country, origins, destinations) {
  ...
  if (!hasOut && !hasIn) {
    box.html(`no bilateral migration data is available for <b>${country}</b> in 2024.`);
    return;
  }
  ...
  let text = `
    migration patterns for <b>${country}</b> in <b>2024</b> show its main 
  ...
  text += `
      ...
      the migration profile of <b>${country}</b>.
  `;
...
function updateTop5(country) {
  if (!TOP5_READY) {
    setTimeout(() => updateTop5(country), 150);
    return;
  }

  const rows = TOP5_CSV.filter(r => r.year === "2024");

  const topOrigins = rows
    .filter(r => r.destination_country_name === country && r.migrants_millions > 0)
    .sort((a,b) => b.migrants_millions - a.migrants_millions)
    .slice(0, 5)
    .map(d => ({
      label: d.origin_country_name,
  ...
  const topDestinations = rows
    .filter(r => r.origin_country_name === country && r.migrants_millions > 0)
    .sort((a,b) => b.migrants_millions - a.migrants_millions)
    .slice(0, 5)
    .map(d => ({
      label: d.destination_country_name,
      migrants_millions: d.migrants_millions,
      origin_subregion: d.origin_subregion,
      destination_subregion: d.destination_subregion
    }));
  ...
  updateTop5CountryText(country, topOrigins, topDestinations);
}
